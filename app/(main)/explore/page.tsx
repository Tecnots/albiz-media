          ...
          <div className="flex gap-1 md:gap-1.5 overflow-x-auto pb-2 -mx-3 px-3 md:-mx-4 md:px-4 lg:-mx-6 lg:px-6">
            {exploreTabs.filter(tab => isSignedIn || tab !== "Followed").map(tab => (
              <button
                ...
                className={`px-2.5 py-1 md:px-3 md:py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${exploreTabs.indexOf(tab) === activeTab
          ...
          <div className="flex gap-1 md:gap-1.5">
            {exploreSubTabs.map((tab, i) => (
              <button
                ...
                className={`px-2.5 py-1 md:px-3 md:py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${i === activeSubTab
                    ? "bg-[#F44444] text-white"
                    : "bg-[#f5f5f5] text-[#525252] hover:bg-[#ebebeb] border border-[#e5e5e5]"
                  }`}
              >
            ...
            {exploreUsers.length === 0 && exploreLoading ? (
              ...
              searchFiltered.map((user, idx) => {
                ...
                return (
                  <div
                    key={user.id}
                    className={`flex items-center gap-2.5 md:gap-3 p-2.5 md:p-3 rounded-xl border transition-colors ${showRank && rank === 1
                        ? "border-[#F44444]/30 bg-[#FFF8F8]"
                        : "border-[#e5e5e5] bg-white hover:border-[#d5d5d5]"
                      }`}
                  >
                    <div className={`w-10 h-10 md:w-11 md:h-11 rounded-full overflow-hidden flex-shrink-0 ${user.hasStory
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 md:px-3 md:py-1.5 text-xs font-medium rounded-full transition-all flex-shrink-0 ${isFollowing
